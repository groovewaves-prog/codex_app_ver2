...
| AI Display Director | レビュー後の次アクション案内を担う唯一のパネル。高重要度・中重要度・構成チェックなどの状態に応じて、最初に見るべき情報をアクション起点で提示。通常画面は修正計画カード中心に整理 |
...
| 章別深堀 | Step 4 の修正計画直下に独立した入口として表示。文書別詳細表示トグルに依存せず、通常モードは最初の深堀候補章のみ有効。開発者モードでは検証用に全章深堀可。最大2パス。追加指摘は修正計画カードへ合流し、文書別詳細側ではサマリ行 + 折りたたみ詳細で経緯確認できる |
...
レビュー結果は以下の順で確認する。AI Display Director がこの順序を画面上で案内する。Step 4 では他の「次の一手」カードは表示しない。

1. Review Result サマリ
2. AI Display Director の案内
3. 修正アクションプラン
4. 必要に応じて追記テンプレート
5. 追加確認が必要なら独立セクション「章別深堀」から対象章をAI再分析
6. 次回比較が必要なら再レビュー用修正計画JSONを保存
7. 必要に応じて障害シナリオと予防策、文書構成チェック詳細、文書別詳細を開く

`Review Result` と修正アクションプランを主導線にし、文書構成チェック詳細や文書別元指摘は折りたたみに退避している。
