...
- **2026-05-01 追加分** (R-L):
  - **R-L** (`models.py`, `rubric.py`, `reviewer.py`, `sanitizer.py`, `streamlit_app.py`, 新 PR): R-K で誤判定が解消した後の社内レビュー評価で「指摘は出ているが、レビューとしては浅い」という評価を受け、出力の構造を全面深化。**広さ(ルーブリックの観点網羅性)と深さ(指摘 1 件あたりの構造化)の両方** を底上げ。
    - **B0 (data model)**: `ReviewSummary` クラスを新設(目的/目的セクション/目的乖離/内容要約/全体評価/総合判定 A-D の 6 フィールド)。`ReviewIssue` に 7 フィールド追加(issue_id / section / current_state / issue / impact / required_timing / re_review_required)。すべてオプショナルで後方互換 100%。
    - **B1 (ルーブリック深化)**: `RUBRICS["design"]` の Mandatory Check を 2 → **5 個** に拡張(traceability / non_functional_coverage / risk_and_open_issues 追加)。各 EvaluationAxis の checkpoint を 3 → 5-7 個に増強。weight 微調整(整合性 20→15、運用保守性 15→20)。`RUBRICS["proposal"]` を新規追加(企画書/提案書、5 EA、weight 合計 100)。`FILENAME_PROPOSAL_KEYWORDS` を新設、ファイル名 signal の優先順位は `design > proposal > change_runbook > operations_runbook`。`OUTBOUND_TEXT_CHARS` デフォルト 12000 → **16000** (拡張ルーブリックでもプロンプトが収まるように)。
    - **B2 (プロンプト & パーサ)**: `SYSTEM_PROMPT` を全面書き直し(役割定義 + 評価方針 + サマリー方針 + JSON 出力指示)。`REVIEW_RESPONSE_SCHEMA` を新スキーマ対応(summary は string/object 両対応、issues に新 6 フィールド追加)。`_parse_review_payload` を 2-tuple → **3-tuple** 化 `(summary_text, summary_struct, issues)`。新旧スキーマ両対応で、新スキーマ時は `details` を `current_state + issue + impact` から自動合成。`_assign_issue_ids` を新設、profile ベースの ID プレフィックス(D-/P-/CR-/OR-/SC-)を Python 側で採番。
    - **B3 (UI 表示)**: `streamlit_app.py` のサマリー表示を 4 セクション + verdict バッジに刷新。指摘表示を 6 フィールド構造化(現状 / 問題点 / 影響 / 推奨対応 / 対応期限バッジ / 再レビュー要否バッジ)。`PROFILE_LABELS` に企画書を追加。すべて旧スキーマ時のフォールバック実装あり(`is_empty()` / `has_structured_fields()` で分岐)。
    - **テスト件数**: 90 → **115 件**（`tests/test_models.py` を新規追加 7 件、`tests/test_rubric_classification.py` に 11 件追加、`tests/test_reviewer.py` に 7 件追加）。`python -m unittest discover tests` で全件通過確認済み。
    - **設計上の制約**: 現状利用可能な LLM が `gemma-4-31b-it` のみのため、設計はモデル非依存にしつつ、プロンプトサイズ(SYSTEM_PROMPT + ルーブリック + 文書本体 = 推定 20000-23000 字)が Gemma 4 の context window に収まるよう配慮。実機での挙動精度は PR マージ後に確認。
...
| プロファイル自動判定 | ファイル名最優先方式（`secure_review/rubric.py`、R-K で改修済み。競合検出時は UI に警告表示。R-L で企画書プロファイル追加） |
| ルーブリック設計 | `secure_review/rubric.py`、R-L で深化済み (design は MC 5 個 + EA 5 軸 / proposal 新規 5 EA / weight 合計 100) |
| 確認ゲート | UI のチェックボックス（`MASK_AND_CONTINUE_REQUIRE_CONFIRM=true`） |
| 外部レビュー LLM | Gemini API 経由の Gemma 4 31B（`gemma-4-31b-it`、JSON モード強制、R-L で構造化サマリと 6 フィールド指摘を出力）|
| レビュー出力構造 | サマリー 4 セクション + 総合判定 A-D / 指摘 6 フィールド (R-L で B0-B3 にて全面深化) |
...
**115 件全通過**(`tests/` 全体を `python -m unittest discover tests` で実行した場合)。

```powershell
python -m unittest discover tests
```

※ `python -m unittest tests.test_reviewer tests.test_sensitivity tests.test_sanitizer tests.test_rubric_classification tests.test_models` のように個別指定で実行した場合は 88 件(うち sanitizer は 16 件で R-H で 4 件 + R-J で 3 件追加済み、reviewer は R-B / R-C で 6 件 + R-L で 7 件追加済み、`test_rubric_classification` は R-K で 14 件 + R-L で 11 件、`test_models` は R-L で 7 件新設)。`discover` 実行では `test_app.py` / `test_env_loader.py` / `test_extractor.py` / `test_network_guard.py` を含めた **115 件** が真値。
...
- `secure_review/sanitizer.py` — R1-R4 + R-H 命名規則 regex + R-J 再マスク防止 + R-L で OUTBOUND_TEXT_CHARS 16000 拡張
- `secure_review/sensitivity.py` — 日本語化済み
- `secure_review/reviewer.py` — JSON モード対応 + R-B/R-C モデル由来サマリ表示 + R-L で SYSTEM_PROMPT 全面書き直し / 構造化サマリパーサ / 指摘 ID 採番
- `secure_review/models.py` — `ReviewResult.raw_response`, `model` フィールド + R-L で `ReviewSummary` 新設 / `ReviewIssue` 7 フィールド拡張
...
│   ├── models.py                   # ReviewResult.raw_response, model (R-B/R-C) + R-L で ReviewSummary 新設 / ReviewIssue 7 フィールド拡張
│   ├── network_guard.py            # R1/R3 中核
│   ├── reviewer.py                 # JSON モード + R-B/R-C モデル由来サマリ + R-L で SYSTEM_PROMPT 全面書き直し / 構造化サマリパーサ / 指摘 ID 採番
│   ├── rubric.py                   # 研究 + テンプレート反映、R-K (ファイル名最優先プロファイル判定 / 競合検出) + R-L (design 大幅深化 / proposal 新規 / 企画書 signal)
│   ├── sanitizer.py                # R-H/R-J 対応済み + R-L で OUTBOUND_TEXT_CHARS 16000 拡張
...
└── tests/
    ...
    ├── test_app.py
    ├── test_env_loader.py
    ├── test_models.py              # R-L で新設（7 件、ReviewSummary / ReviewIssue 拡張のテスト）
    ├── test_network_guard.py
    ├── test_reviewer.py            # R-B/R-C で 6 件 + R-L で 7 件追加（合計 32 件）
    ├── test_rubric_classification.py  # R-K で 14 件 + R-L で 11 件（合計 25 件）
