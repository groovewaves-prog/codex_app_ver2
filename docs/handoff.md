...
| 修正アクションプラン | レビュー指摘、文書構成チェック、章別深堀指摘から、修正方針、追記テンプレート、再レビュー条件をローカル生成。今回レビュー結果JSONを次回比較台帳として保存可能 |
...
- 深堀指摘は `origin` で初回レビューと区別し、修正アクションプランにも合流する。独立した「📌 深堀結果」表示は D-2 step3 で整理予定。
