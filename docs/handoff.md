...
- G-2 で Step 4（レビュー結果）は結論ファースト構成へ再構築済み。`N 件の指摘`、`対応すべき指摘`カード、`補助で見るもの`を主導線とし、AI Display Director / per-document 詳細トグルは画面経路から外している。
- 章再分析は、指摘カードに章を紐づけられる場合だけカード内に表示し、紐づかない章は補助セクション `章単位の追加レビュー` から直接選択する。
...
| 領域 | 現状 |
|---|---|
| UI | Streamlit。日本語UI。ヒーロー表示、AI Operation Co-Pilot、ステータスバー、Step 2 の送信前匿名化確認UI、Step 4 の結論ファースト型カードUI、折りたたみ・スクロール表示を実装済み。G-1 の `--sr-*` デザイントークンと共通HTMLコンポーネントを Step 2/4 で利用開始 |
...
| AI Operation Co-Pilot | ローカル状態から、次に押す操作を提示。常時表示はステップ、見出し、次アクションに絞り、理由、完了条件、注意点、チェックリストは折りたたみ詳細へ退避。外部LLMは呼び出さない決定論的な操作アシスト |
| Step 4 v2 | AI Display Director / 文書別詳細トグルを画面経路から外し、`N 件の指摘`、`対応すべき指摘`カード、`補助で見るもの` に再構成。指摘カードが主役で、構成チェック・章単位追加レビュー・将来リスク・使い方は補助セクションに集約 |
...
| 修正アクションプラン | レビュー指摘、文書構成チェック、章別深堀指摘から、修正方針、追記テンプレート、再レビュー条件をローカル生成。追記テンプレートの「現状」は LLM の `current_state`、details 内の `【現状】`、またはユーザ向け確認誘導文で補完。深堀由来のカードには origin バッジを表示。再レビュー用の修正計画JSONを次回比較台帳として保存可能 |
...
| 章別深堀 | 指摘カードに章を紐づけられる場合はカード内に `章を再分析` ボタンを表示。紐づかない章は `補助で見るもの` の `章単位の追加レビュー` から直接選択。最大2パス。追加指摘は修正計画カードへ合流 |
...
ステータスバーは Co-Pilot の直下に1回だけ表示し、以下の状態を表示する。
...
- 匿名化結果を再生成
- 匿名化後テキスト確認
- レビュー結果側の証跡・エクスポートからレビュー証跡(JSON)を保存
...
1. `N 件の指摘` の大数字サマリ
2. 重要度チップ（高 / 中 / 不足章 / 将来リスク）
3. `対応すべき指摘` カード
4. 必要に応じてカード内の追記テンプレートと `章を再分析`
5. 次回比較が必要なら再レビュー用修正計画JSONを保存
6. 必要に応じて `補助で見るもの` から文書構成チェック、章単位の追加レビュー、将来リスク、使い方を開く

`対応すべき指摘` を主導線にし、文書構成チェックや章単位レビューは機能別の補助セクションへ退避している。
...
| 中 | 長文・複数ファイル時の待ち時間、分割call、クォータ挙動の実地確認 | 一部対応済み。文書別トークン、最低待機時間、推奨分割案をUI表示するよう改善。実地確認は継続 |
...
2026-05-26 時点では、追記テンプレート「現状」欄に内部指示文が表示される問題も修正済み。
