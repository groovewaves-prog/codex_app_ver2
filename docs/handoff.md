...
| 領域 | 現状 |
|---|---|
| UI | Streamlit。日本語UI。ヒーロー表示、AI Operation Co-Pilot、レビュー管制エージェント、ステータスバー、送信前チェック、カード型サマリ、折りたたみ・スクロール表示を実装済み |
...
| 修正アクションプラン | レビュー指摘と文書構成チェックから、修正方針、追記テンプレート、再レビュー条件をローカル生成。再レビュー比較モード時のみ、次回比較台帳としてJSON保存を表示 |
| 再レビュー比較 | 「前回文書の再レビューを使う」をオンにした場合だけ、前回の修正計画JSONを読み込み、今回アップロードした修正文書に改善要素が反映されているかをローカル照合。追加LLM送信なし |
| 先読みレビュー | 曖昧表現の未確定、読み手別誤読リスク、未来障害プレモーテム、メタレビューをローカル補助解析として表示。プレモーテムは本文確認済み・本文不足・レビュー指摘ヒントを分離。追加LLM送信なし |
