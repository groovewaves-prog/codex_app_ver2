...
| AI Display Director | レビュー後の次アクション案内を担う唯一のパネル。レビュー結果の状態から、今見る情報、必要なときだけ開く情報、開発者モードだけで見る情報をローカル判定。通常画面は修正計画カード中心に整理 |
...
| 修正アクションプラン | レビュー指摘、文書構成チェック、章別深堀指摘から、修正方針、追記テンプレート、再レビュー条件をローカル生成。深堀由来のカードには origin バッジを表示。今回レビュー結果JSONを次回比較台帳として保存可能 |
