...
| AI Display Director | レビュー後の次アクション案内を担う唯一のパネル。レビュー結果の状態から、今見る情報、補助で見る情報、開発者モードだけで見る情報をローカル判定。通常画面は修正計画カード中心に整理 |
...
| 先読みレビュー | 品質改善ヒントとして必要時だけ開く。曖昧表現の未確定、読み手別誤読リスク、未来障害プレモーテムをローカル補助解析。メタレビューは開発者モードのみ。追加LLM送信なし |
| 文書全体の概要 | LLMまたはローカル補完により、目的・内容要約・全体評価を表示 |
| 章別概要レビュー | 全章を章カードで表示。概要レビューと深堀候補を分離 |
| 章別深堀 | 通常モードは最初の深堀候補章のみ有効。開発者モードでは検証用に全章深堀可。最大2パス。追加指摘は修正計画カードへ合流し、章内の独立表示はサマリ行 + 折りたたみ詳細に縮退 |
...
6. 必要に応じて品質改善ヒント、文書構成チェック詳細、文書別詳細、章別深堀候補を開く
