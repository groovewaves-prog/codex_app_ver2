...
| 領域 | 現状 |
|---|---|
| UI | Streamlit。日本語UI。ヒーロー表示、AI Operation Co-Pilot、レビュー管制エージェント、AI Display Director、ステータスバー、送信前チェック、カード型サマリ、折りたたみ・スクロール表示を実装済み |
...
| レビュー管制エージェント | ローカル状態からミッション、次アクション、監視ポイント、取込・匿名化・送信判断・レビュー段階を表示。外部LLMは呼び出さない決定論的UI |
| AI Display Director | レビュー結果の状態から、今見る情報、必要なときだけ開く情報、開発者モードだけで見る情報をローカル判定。通常画面は修正計画カード中心に整理 |
...
| 修正アクションプラン | レビュー指摘と文書構成チェックから、修正方針、追記テンプレート、再レビュー条件をローカル生成。今回レビュー結果JSONを次回比較台帳として保存可能 |
...
レビュー結果は以下の順で確認する。AI Display Director がこの順序を画面上で案内する。
