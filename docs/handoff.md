...
| 領域 | 現状 |
|---|---|
| UI | Streamlit。日本語UI。ヒーローと AI Operation Co-Pilot の画面経路は撤去済み。Step 1/2/3/4 は status bar + 各Step専用 v2 UI で構成し、Step 2 の送信前匿名化確認、Step 3 の外部送信境界確認、Step 4 の結論ファースト型カードUIを実装済み。G-1 の `--sr-*` デザイントークンと共通HTMLコンポーネントを段階的に利用 |
...
| AI Operation Co-Pilot | G-4 で画面経路から撤去済み。レビュー中・完了・エラー時も direct status bar で状態を示す。`secure_review.agent_planner` 側の旧ロジック本体は G-5 で最終整理予定 |
