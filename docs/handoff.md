...
| 文書アップロード | PDF / DOCX / XLSX / PPTX / TXT 等を受付。複数PDFは「同一種類の文書を複数ファイルで構成」として一括レビュー。Office/PDF は抽出形式メタ情報と抽出上の注意をLLMプロンプトへ渡す |
...
- 2026-05-21 時点で `python -m unittest` は **294 tests OK**。
