...
# チェックリスト評価 (Phase 5: 構造定義書 v0.2)

別途プロンプトで提示される「構造定義書 v0.2 の 15 章チェックリスト」(IPA + AWS WAF + ISO/IEC 25010 ベース) について、各文書ごとに **その文書に該当する全項目** を 5 段階で評価してください。

- status は以下のいずれか:
  - "excellent": 模範的、他プロジェクトの参考になる記載
  - "good": 充足、業界標準を満たしている
  - "acceptable": 最低限充足、改善の余地あり
  - "needs_improvement": 部分的欠落・曖昧な記載、修正推奨
  - "unacceptable": 重大な欠落・誤り、是正必須
  - "not_applicable": この文書には該当しない (例: 設計書全体構成の項目をネットワーク単一文書に当てはめない)

- reason: なぜそう評価したかを 1〜2 文で簡潔に説明 (必須)
- evidence: 該当箇所の章番号や見出し (例: "1.1 本書の位置付け")。文書全体に分散している場合は "全体" と記述。特定不能なら空文字列。
- source_document: 評価対象の文書のファイル名 (必須)
- item_id: チェックリストの項目 ID (例: "1.1") を必ず正確にコピーすること
- item_name: チェックリストの項目名 (例: "本書の目的") を必ず正確にコピーすること

文書に該当しない項目は status="not_applicable" として返してもよいし、配列から省略してもよい。

# 欠落章サジェスチョン (Phase 5)

- 単一文書のレビュー (この呼び出し) では、missing_chapters は **空配列 []** を返してください。
- 全文書集約後の別の呼び出し (集約 call) で、文書群全体としての欠落章を判定します。
"""
...
class HttpLlmReviewProvider(ReviewProvider):
    ...
    def review(
        ...
        *,
        ...
        # Phase 5 (2026-05-08): chunking 無効時 (単一 call) でも
        # checklist_results を抽出する (LLM が返してくれば反映)。
        # missing_chapters は単一 call 時には基本的に空配列を返すよう
        # SYSTEM_PROMPT で指示しているが、念のためパースしておく。
        single_doc_name = documents[0].name if len(documents) == 1 else ""
        checklist_results = _parse_checklist_results(content, single_doc_name)
...
class GeminiApiReviewProvider(ReviewProvider):
    ...
    def _review_chunked(
        ...
        *,
        ...
        # Phase 5 (2026-05-08): 各 chunk call で得た checklist_results を集約
        all_checklist_results: list[ChecklistResult] = []
        per_doc_summaries: list[str] = []
        per_doc_raw_responses: list[str] = []
        per_doc_prompts: list[str] = []
        per_doc_summaries_for_aggregate: list[str] = []  # Phase 5 集約 call 用
        ...
        for idx, doc in enumerate(documents, start=1):
            ...
            try:
                ...
                # Phase 5 (2026-05-08): この文書のチェック評価を抽出
                # source_document が空の場合は文書名で fallback
                doc_checklist = _parse_checklist_results(content, doc_name)
                all_checklist_results.extend(doc_checklist)
                if doc_summary:
                    per_doc_summaries.append(f"【{doc_name}】{doc_summary}")
                    # 集約 call に渡す簡潔サマリ (cap して入力 token を抑える)
                    per_doc_summaries_for_aggregate.append(
                        f"- {doc_name}: {doc_summary[:200]}"
                    )
        ...
        # Phase 5 (2026-05-08): 集約 call (missing_chapters の判定)
        # Q18=A: 全文書完了後に 13 番目の API call を発行し、文書群全体として
        # 欠落している章 (構造定義書 v0.2 の 15 章基準) を判定。
        # design プロファイルかつ全文書失敗でない場合のみ実行。
        missing_chapters: tuple = ()
        if (
            rubric.document_profile == "design"
            and len(failed_docs) < total
            and per_doc_summaries_for_aggregate
        ):
            if progress_callback is not None:
                try:
                    progress_callback(total, total, "(集約) 欠落章を判定中...")
                except Exception:  # noqa: BLE001
                    pass
            try:
                aggregate_prompt = self._build_aggregate_prompt(
                    documents, rubric, per_doc_summaries_for_aggregate
                )
                aggregate_payload = self._build_payload(aggregate_prompt)
                aggregate_response = self._post_with_retry(aggregate_payload)
                aggregate_content = _extract_gemini_text(aggregate_response)
                missing_chapters = _parse_missing_chapters(aggregate_content)
                # 集約 call の生レスポンスも raw_response に含めて開発者モードで確認可能に
                per_doc_raw_responses.append(
                    f"=== (集約 call: missing_chapters 判定) ===\n{aggregate_content}"
                )
            except RuntimeError as exc:
                # 集約 call の失敗は致命的ではない (個別 chunk の結果は残る)
                LOGGER.warning(
                    "Phase 5 aggregate call failed (missing_chapters not available): %s",
                    exc,
                )
                per_doc_raw_responses.append(
        ...
        return _build_review_result(
            summary=summary,
            summary_structured=None,  # Phase 5: 集約 call は missing_chapters のみ取得 (summary は per-doc から構築)
            issues=all_issues,
            provider=self.name,
            documents=documents,
            rubric=rubric,
            ...
            model=self.model,
            checklist_results=tuple(all_checklist_results),
            missing_chapters=missing_chapters,
        )
    ...
    def _build_aggregate_prompt(
        ...
        documents: list[SanitizedDocument],
        rubric: ReviewRubric,
        per_doc_summaries_for_aggregate: list[str],
    ) -> str:
        """Phase 5 (2026-05-08): 集約 call 用プロンプトを構築。

        各文書 chunk の簡潔サマリ + 文書名一覧を入力し、構造定義書 v0.2 の
        15 章のうち欠落している章 (missing_chapters) を判定させる。
        各 chunk の本文 (full text) は送らないので、入力 token を抑えられる。
        """
        chapter_list_text = render_chapter_checklist_for_prompt(DESIGN_DOC_STRUCTURE_V0_2)
        doc_names = "\n".join(f"  - {d.name}" for d in documents)
        sections = [
            "次の文書群を全体としてレビュー済みです。各文書のサマリと文書名一覧から、",
            "構造定義書 v0.2 の 15 章構造のうち **どの章が文書群全体として欠落しているか** を判定してください。",
            "",
            "## 提供文書一覧",
            doc_names,
            "",
        ...
        sections.extend(per_doc_summaries_for_aggregate)
        sections.append("")
        sections.append("## 構造定義書 v0.2 の 15 章")
        sections.append(chapter_list_text)
        sections.append("")
        sections.append("## 出力指示")
        sections.append("")
        sections.append(
            "上記文書群が **明らかにカバーしていない章** について、"
            "missing_chapters 配列で判定してください。"
        )
        sections.append("")
        sections.append("verdict は以下のいずれか:")
        sections.append("- should_have: 本来必要、欠落として指摘すべき (例: 設計書なのに性能設計が欠落)")
        sections.append("- recommended: あればよい、サジェスチョンとして推奨 (例: コスト設計、移行設計)")
        sections.append("- out_of_scope: スコープ外として黙認 (例: 要件定義書フェーズの内容)")
        sections.append("")
        sections.append("各 missing_chapter には以下を必ず含める:")
        sections.append("- chapter_id (例: ch9)")
        sections.append("- chapter_name (例: 性能設計)")
        sections.append("- verdict")
        sections.append("- justification: なぜその verdict か (1〜2 文)")
        sections.append("- suggested_content: もし作成するなら、本来書かれるべき内容 (3〜5 行の要点)")
        sections.append("")
        sections.append(
            "issues 配列と checklist_results 配列は **空配列 []** を返してください "
            "(個別文書のレビュー結果は別途集約済みのため)。"
        )
        sections.append(
            "summary オブジェクトも **全フィールド空文字列 + verdict='B'** で返してください "
            "(個別文書のサマリは既に集計済みのため)。"
        )
        sections.append("")
        sections.append("出力は必ず JSON オブジェクトのみで返してください。")
        return "\n".join(sections)
...
def build_prompt(
    ...
    *,
    ...
    sections = [
        ...
        render_rubric_for_prompt(rubric),
        "",
        # Phase 5 (2026-05-08): 構造定義書 v0.2 ベースのチェックリストを追加
        # design プロファイル (= 設計書) のときのみ埋め込む。手順書・コードでは
        # この 15 章構造は適用しないため、無関係なチェックリストを送らない。
        "",
        "## 構造定義書 v0.2 のチェックリスト (5 段階評価対象)",
        "",
        "次のチェックリスト項目について、各文書ごとに該当する全項目を 5 段階で評価し、",
        "checklist_results 配列に格納してください (システムプロンプトの指示に従う)。",
        "",
        render_chapter_checklist_for_prompt(DESIGN_DOC_STRUCTURE_V0_2)
        if rubric.document_profile == "design"
        else "(このプロファイルではチェックリスト評価は不要です。checklist_results は空配列で返してください。)",
        "",
        "出力は必ず JSON オブジェクトのみで返してください。markdown のコードブロックや前置きを付けないこと。",
        "JSON の構造はシステムプロンプトで指定した形式に従ってください。",
        "missing_chapters は **空配列 []** を返してください (集約 call で別途判定するため)。",
    ]
